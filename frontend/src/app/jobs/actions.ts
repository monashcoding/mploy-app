    ...
    return await withDbConnection(async (client) => {
        ...
        if (minSponsors == 0) {
            ...
            return {
                ...
                jobs: (jobs as MongoJob[]).map(serializeJob).map((job) => ({ ...job, highlight: false })),
                total,
            };
            ...
            let sponsoredJobs = await collection.aggregate([{ $match: sponsoredQuery }, { $sample: { size: minSponsors * 8 } }]).toArray();
            ...
            const [otherJobs, total] = await Promise.all([
                collection.find(filteredQuery).skip(skip).limit(PAGE_SIZE - sponsoredJobs.length).toArray(),
            ...
            const mergedJobs = [...sponsoredJobs.map((job) => ({ ...job, highlight: true })), ...otherJobs.map((job) => ({ ...job, highlight: false }))].slice(0, PAGE_SIZE);
